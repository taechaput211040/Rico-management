            ...
            <v-card width="100%">
              <v-expansion-panel>
                <v-expansion-panel-header @click="refreshform()">
                  <v-card-title primary-title class="font-weight-bold grey--text">
                    เติมเงิน
                  </v-card-title>
                ...
                <v-expansion-panel-content>
                  <div>
                    กรอกusername
                    <v-text-field class="mb-2" dense outlined v-model="formEditCredit.username" clearable
                      placeholder="เช่น xx1234567" hide-details="auto"></v-text-field>
                    กรอกจำนวนเงิน
                    ...
                    หมายเหตุ (ไม่บังคับ)
                    <v-text-field class="mb-2" dense v-model="formEditCredit.remark" outlined clearable
                      placeholder="หมายเหตุ (ไม่บังคับ)" hide-details="auto"></v-text-field>
                    <span class="error--text">หมายเหตุ: *** เติมเครดิตในหน้านี้
                      ใช้ในกรณียอดไม่แสดงในระบบ จะทำการ เติม
                      เครดิตให้ลูกค้าตามโปรโมชั่นที่ลูกค้ารับอยู่
                      คิดเป็นจำนวนยอดฝากจริง ***</span>

                    <v-card-actions>
                      <v-btn color="primary" :disabled="canwrite" class="ma-auto"
                        @click="editTopupCredit()">เติมเงิน</v-btn>
                    </v-card-actions>
                  </div>
                </v-expansion-panel-content>
              </v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  <div>
                    ...
                    <v-text-field class="mb-2" dense outlined v-model="formEditCredit.username" clearable
                      placeholder="เช่น xx1234567" hide-details="auto"></v-text-field>
                    จำนวนเครดิตที่จะตัด
                    <v-text-field class="mb-2" dense v-model="formEditCredit.credit" outlined type="number"
                      placeholder="กรอกจำนวนเงิน" hide-details="auto"></v-text-field>
                    หมายเหตุ
            ...
            <v-card width="100%">
              <v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  <div>
                    กรอกusername
                    <v-text-field class="mb-2" dense outlined clearable v-model="formEditCredit.username"
                      placeholder="เช่น xx1234567" hide-details="auto"></v-text-field>
                    โบนัสเครดิต (ไม่คิดคำนวนในยอดฝาก)
                    <v-text-field class="mb-2" dense v-model="formEditCredit.bonus" outlined type="number"
                      placeholder="กรอกจำเครดิต" hide-details="auto"></v-text-field>
                    หมายเหตุ (ไม่บังคับ)
                    <v-text-field class="mb-2" v-model="formEditCredit.remark" dense outlined clearable
                      placeholder="หมายเหตุ (ไม่บังคับ)" hide-details="auto"></v-text-field>
                    <div class="card-turnsetting pa-2">
                      ...
                      <v-row>
                        <v-col cols="6">
                          <v-text-field dense v-model="formEditCredit.turn.tslot" label="เทิร์น SLOT" outlined clearable
                            type="number" hide-details="auto"></v-text-field></v-col>
                        <v-col cols="6">
                          <v-text-field v-model="formEditCredit.turn.tsportbook" label="เทิร์น SPORTBOOK" dense outlined
                            type="number" clearable hide-details="auto"></v-text-field></v-col>
                        <v-col cols="6">
                          <v-text-field v-model="formEditCredit.turn.tesport" label="เทิร์น ESPORT" dense outlined
                            clearable type="number" hide-details="auto"></v-text-field></v-col>
                        <v-col cols="6">
                          <v-text-field dense v-model="formEditCredit.turn.tother" label="เทิร์น HORSE RACING" outlined
                            clearable type="number" hide-details="auto"></v-text-field></v-col>
                        <v-col cols="6">
                    ...
                    <span class="error--text">หมายเหตุ: *** การแก้ไขเครดิตในหน้านี้ จะทำการ เติม โบนัส
                      ...
                      ***</span>
                    <v-card-actions>
                      <v-btn :disabled="canwrite" color="primary" class="ma-auto"
                        @click="editTopupBonus()">เติมโบนัส</v-btn>
                    </v-card-actions>
                  </div>
          ...
          <v-data-table show-expand hide-default-footer class="elevation-1" :headers="header" :items="itemcredit"
            ...
            <template #[`item.data-table-expand`]="{ isExpanded, expand }">
              <div class="px-2">
                ...
                <v-btn @click="expand(false)" v-if="isExpanded" color="black" dark small>ปิด</v-btn>
              </div>
            </template>
      ...
      <v-card>
        ...
        <v-card-text class="font-weight-bold">
          ...
          <div class="my-2" v-if="formEditCredit.username">
            <v-text-field placeholder="กรอก username : " hide-details="auto" v-model="formEditCredit.username" dense
              outlined required disabled></v-text-field>
          </div>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" small @click="editTopupCredit_confirm()"> ยืนยัน</v-btn>
          <v-btn color="red" small @click="dialogEditTopup = false"> ยกเลิก</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="dialogCutcredit" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" small @click="editCutcredit_confirm()"> ยืนยัน</v-btn>
          <v-btn color="red" small @click="dialogCutcredit = false"> ยกเลิก</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="dialogEditBonus" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" small @click="editTopupBonus_confirm()"> ยืนยัน</v-btn>
          <v-btn color="red" small @click="dialogEditBonus = false"> ยกเลิก</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-flex>
