    ...
    <v-row>
      <v-col cols="12" md="12" lg="4">
        <v-card width="100%" class="elevation-3 rounded-lg pa-2 mb-5">
          ...
          <v-data-table :headers="logFailedColumn" :items="incomingSMS" :items-per-page="20" >
      ...
      <v-col cols="12" md="12" lg="4">
        <v-card width="100%" class="elevation-3 rounded-lg pa-2">
          ...
          <v-data-table :headers="depositColumn" :items="dplist" :items-per-page="5">
      ...
      <v-col cols="12" md="12" lg="4">
        <v-card width="100%" class="elevation-3 rounded-lg pa-2">
          ...
          <v-data-table :headers="withdrawColumn" :items="wdlist" :items-per-page="20">
            <template #[`item.no`]="{ item }">
              <div class="text-center my-3 my-md-2 showdetail">
            ...
            <template #[`item.credit`]="{ item }">
              <div class="text-center my-3 my-md-2 showdetail">
                ...
                <v-chip class="ma-2 font-weight-bold" color="error" label outlined>{{ item.amount }} บาท</v-chip><br />
                ...
                <v-icon color="error">mdi-menu-down{{ item.afAmount | dateFormat }}</v-icon>
              </div>
            </template>
            <template #[`item.actions`]="{ item }">
              ...
              <div v-if="item.status == 'Error'" class="ma-2 text-center">
                <v-btn color="error" class="my-1" rounded small>ไม่อนุมัติ</v-btn>
                <v-btn v-if="item.bankName != 'TRUEWALLET'" color="primary" class="my-1" rounded small>Reset</v-btn>
                <v-btn color="grey" class="my-1" rounded small  @click="manualWd(item)">ถอนมือ</v-btn>
                <v-btn color="primary" outlined rounded class="my-1" small @click="showRemark(item)">หมายเหตุ</v-btn>
              </div>
              <div v-if="item.status == 'created'" class="ma-2 text-center">
                <v-btn v-if="item.bankName != 'TRUEWALLET'" color="success" class="my-1" rounded small>อนุมัติ</v-btn>
                <v-btn color="error" class="my-1" rounded small>ไม่อนุมัติ</v-btn>
                <v-btn color="grey" class="my-1" rounded small @click="manualWd(item)">ถอนมือ</v-btn>
                <v-btn color="primary" outlined rounded class="my-1" small @click="showRemark(item)">หมายเหตุ</v-btn>
              </div>
    ...
    <v-dialog v-model="dialogTopup_hide" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="red" small @click="dialogTopup_hide = false"> ยกเลิก</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialogManualWD" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" small @click="manualWd_confirm()"> ยืนยัน</v-btn>
          <v-btn color="success" small @click="dialogManualWD = false">ยกเลิก</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...mapActions("auth", ["manualWithdrawFromDashboard","hideIncomingFromDashBoardByOperator","GetInfomation", "Autostatus", "checkBalanceBank", "updateBalanceBank", "updateAutoBankStatus", "topupDashboardByOperator", "updateIncoming"]),
    ...mapMutations("auth", ["update_action_bank"]),

 manualWd(item){
  this.withdraw_temp = item,
  this.dialogManualWD = true
    ...
    async hide_incoming(){
      this.isLoading = true
      const result = await this.hideIncomingFromDashBoardByOperator(this.incoming_dashboard)
      this.isLoading = false
      if (result.status == 'success') {
        this.isLoading = false
        this.$swal({
