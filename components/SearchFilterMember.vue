        ...
        <v-row>
          <v-col cols="12">
            <v-row align="baseline">
              ...
              <v-col cols="7">
                <el-date-picker class="full-width" v-model.trim="filter.startDate" arrow-control placeholder="วันที่"
                  style="width: 100%" />
              </v-col>
              <v-col>
                <v-select label="เลือกค้นหา" v-model="filter.options" hide-details="auto" dense outlined
                  :items="options_item"></v-select>
              </v-col>
            </v-row>
          </v-col>
          <v-col cols="12">
            <v-row align="baseline">
              ...
              <v-col cols="7">
                <el-date-picker class="full-width" v-model.trim="filter.endDate" arrow-control placeholder="วันที่"
                  style="width: 100%" />
              </v-col>
              <!-- <v-col>
      ...
      <v-col cols="12" md="6" lg="6">
        <v-row>
          <v-col cols="8" md="8" v-if="searchinput == true">
            <v-text-field label="ค้นหาด้วย username" placeholder="กรอกคำค้นหา" dense v-model.trim="filter.inputfilter"
              outlined clearable hide-details="auto"></v-text-field></v-col>
          ...
          <v-col cols="8" class="d-flex">
            <v-btn color="success" class="mx-1" dark @click="toyesterday()"><v-icon left>mdi-menu-left</v-icon>
              เมื่อวาน</v-btn><v-btn color="warning" class="mx-1" @click="today()">วันนี้<v-icon
                right>mdi-circle-medium</v-icon></v-btn>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
  ...
  data() {
    return {
      ...
      options_item: [
        ...
        { text: 'เลขอ้างอิงบัญชี', value: 'bankRef' },
      ]
    };
  },
