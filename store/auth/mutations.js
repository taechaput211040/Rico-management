...
export function set_dashboard(state, payload) {
  ...
  state.datarander.profitlossDate =
    payload.dpamountoneday.amount - payload.wdamountoneday.amount;
...
export function set_logout(state, context) {
  ...
  context.rootState.setting.activity = null;
...
export function addDeposit(state, payload) {
  ...
  payload.smsdatetime = dayjs(payload.smsdatetime).format(
    "YYYY-MM-DD HH:mm:ss"
  );
...
export function set_statichash(state, payload) {
  localStorage.setItem("statichash", payload.data.hash);
  state.statichash = payload.data.hash;
}
